...
namespace SIPSorcery.Net
{
    ...
    public class RtpIceChannel : RTPChannel
    {
        ...
        /// <summary>
        /// The period in seconds a nominated candidate pair may stay without a confirming
        /// success response before the connection is failed outright. Matches the short
        /// patience window typical of mobile peers; the stock Succeeded-timeout path below
        /// only runs after FAILED_TIMEOUT_PERIOD which is far too late to recover.
        /// </summary>
        public static int NOMINATED_CONFIRM_TIMEOUT = 7;
                    ...
                    lock (_checklistLock)
                    {
                        if (_checklist.Count > 0)
                        {
                            ...
                            else
                            {
                                ...
                                // NEKTO PATCH (nomination stall fix): a nominated entry whose
                                // use-candidate check never got a success response stays in the
                                // Succeeded state and is invisible to the Waiting/InProgress
                                // retransmit logic above. The single use-candidate check was the
                                // only one ever sent, so if that one request or its response is
                                // lost the nomination deadlocks: no NominatedEntry, no DTLS, and
                                // the only recovery is the FAILED_TIMEOUT_PERIOD (16s) scan below
                                // which fires long after the remote peer has hung up. Re-send the
                                // use-candidate check at the standard RTO so one lost message
                                // cannot stall the connection, and fail fast once the nominated
                                // pair has stayed unconfirmed for NOMINATED_CONFIRM_TIMEOUT so
                                // the application can rotate instead of waiting for the peer.
                                if (NominatedEntry == null && IceGatheringState == RTCIceGatheringState.complete)
                                {
                                    var nominatedUnconfirmed = _checklist.FirstOrDefault(x => x.Nominated
                                        && x.State == ChecklistEntryState.Succeeded
                                        && x.LastCheckSentAt > DateTime.MinValue);

                                    if (nominatedUnconfirmed != null)
                                    {
                                        if (DateTime.Now.Subtract(nominatedUnconfirmed.FirstCheckSentAt).TotalSeconds > NOMINATED_CONFIRM_TIMEOUT)
                                        {
                                            logger.LogWarning("ICE RTP channel nominated candidate pair was not confirmed after {Timeout}s, failing the connection: {LocalCandidate}->{RemoteCandidate}.",
                                                NOMINATED_CONFIRM_TIMEOUT, nominatedUnconfirmed.LocalCandidate.ToShortString(), nominatedUnconfirmed.RemoteCandidate.ToShortString());
                                            nominatedUnconfirmed.State = ChecklistEntryState.Failed;
                                            nominatedUnconfirmed.Nominated = false;
                                            IceConnectionState = RTCIceConnectionState.failed;
                                            return;
                                        }

                                        if (DateTime.Now.Subtract(nominatedUnconfirmed.LastCheckSentAt).TotalMilliseconds > rto)
                                        {
                                            logger.LogDebug("ICE RTP channel re-sending use-candidate check for unconfirmed nominated pair: {LocalCandidate}->{RemoteCandidate}.",
                                                nominatedUnconfirmed.LocalCandidate.ToShortString(), nominatedUnconfirmed.RemoteCandidate.ToShortString());
                                            SendConnectivityCheck(nominatedUnconfirmed, true);
                                            return;
                                        }
                                    }
                                }
